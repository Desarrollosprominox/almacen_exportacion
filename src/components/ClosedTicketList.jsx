...
function ClosedTicketList({ tickets, onRefresh }) {
  ...
  // Filtrar explícitamente solo los tickets con estado "Resuelta"
  const [filteredByStatusTickets, setFilteredByStatusTickets] = useState([]);

  // Verificar que los tickets tengan el estado "Resuelta" y filtrarlos
  useEffect(() => {
    console.log('ClosedTicketList recibió tickets:', tickets);
    if (tickets && tickets.length > 0) {
      // Filtrar explícitamente solo los tickets con estado "Resuelta"
      const onlyResolvedTickets = tickets.filter(ticket => ticket.amv_estado === "Resuelta");
      console.log('Tickets con estado "Resuelta":', onlyResolvedTickets.length);
      
      // Verificar si hay tickets con otro estado que se están descartando
      const otherStateTickets = tickets.filter(ticket => ticket.amv_estado !== "Resuelta");
      if (otherStateTickets.length > 0) {
        console.warn('Se están filtrando tickets con estado diferente a "Resuelta":', otherStateTickets.length);
      }
      
      // Actualizar el estado local con solo los tickets resueltos
      setFilteredByStatusTickets(onlyResolvedTickets);
    } else {
      setFilteredByStatusTickets([]);
    }
  }, [tickets]);
  ...
  const uniqueCategories = useMemo(() => {
    if (!filteredByStatusTickets || filteredByStatusTickets.length === 0) return [];
    const categories = filteredByStatusTickets
      .map(ticket => ticket.amv_categoriadelasolicitud)
      .filter(Boolean)
      .filter((value, index, self) => self.indexOf(value) === index);
    return categories;
  }, [filteredByStatusTickets]);
  
  // Extract unique creators from tickets
  const uniqueCreators = useMemo(() => {
    if (!filteredByStatusTickets || filteredByStatusTickets.length === 0) return [];
    const creators = filteredByStatusTickets
  ...
  const uniqueSucursales = useMemo(() => {
    if (!filteredByStatusTickets || filteredByStatusTickets.length === 0) return [];
    const sucursales = filteredByStatusTickets
  ...
  const activeFilterCount = useMemo(() => {
    ...
    if (searchTerm) count++;
    return count;
  }, [priorityFilter, categoryFilter, creatorFilter, sucursalFilter, startDate, endDate, searchTerm]);

  const filteredTickets = useMemo(() => {
    // Start with search term filter
    let filtered = filteredByStatusTickets;
    ...
    if (startDate) {
      ...
      filtered = filtered.filter(ticket => {
        const ticketDate = new Date(ticket.amv_fechadecierre || ticket.createdon);
    ...
    return filtered;
  }, [filteredByStatusTickets, searchTerm, priorityFilter, categoryFilter, creatorFilter, sucursalFilter, startDate, endDate]);
  ...
  const sortedAndFilteredTickets = useMemo(() => {
    ...
    if (sortField === 'createdon') {
      result.sort((a, b) => {
        const dateA = new Date(a.amv_fechadecierre || a.createdon);
        const dateB = new Date(b.amv_fechadecierre || b.createdon);
    ...
    <div className="bg-white shadow-sm rounded-lg overflow-hidden">
      <div className="p-4 border-b">
        <div className="flex justify-between items-center">
          <h2 className="text-lg font-semibold text-gray-800">Tickets Resueltos</h2>
          <div className="flex gap-2">
            <div className="relative">
              ...
              <input
                ...
                className="pl-10 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#003594]"
                value={searchTerm}
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200 animate-fadeIn">
            ...
            <div className="mt-4 pt-3 border-t border-gray-200 flex justify-between items-center">
              <div className="text-sm text-gray-500">
                Mostrando <span className="font-semibold">{filteredTickets.length}</span> de <span className="font-semibold">{filteredByStatusTickets.length}</span> tickets
                {sortField === 'createdon' && (
