...
import ClosedTicketList from '../components/ClosedTicketList';
import Sidebar from '../components/Sidebar';

function Users() {
  const { isAuthenticated, login } = useAuth();
  const { fetchClosedTickets } = useDataverseService();
  ...
  const loadTickets = useCallback(async () => {
    try {
      setLoading(true);
      const data = await fetchClosedTickets();
      setTickets(data);
      setError(null);
    } catch (error) {
    ...
    <div className="min-h-screen">
      ...
      >
        <header className="bg-white border-b border-gray-200 px-8 py-6">
          <h1 className="text-2xl font-semibold text-gray-900">Solicitudes atendidas</h1>
        </header>
        ...
        <div className="p-8 max-w-7xl mx-auto">
          ...
          {loading ? (
            ...
            <ClosedTicketList tickets={tickets} onRefresh={handleRefresh} />
          )}
        </div>
      </main>
    </div>
